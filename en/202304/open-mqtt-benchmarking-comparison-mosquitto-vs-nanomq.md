> The blog post [Open MQTT Benchmark Suite: The Ultimate Guide to MQTT Performance Testing](https://www.emqx.com/en/blog/open-mqtt-benchmark-suite-the-ultimate-guide-to-mqtt-performance-testing) introduced the Open MQTT Benchmark Suite developed by EMQ. In [the GitHub project](https://github.com/emqx/mqttbs) we defined MQTT benchmark scenarios, use cases and the observation metrics. Based on the activity and popularity of the community and github project, the top 4 open-source MQTT brokers in 2023 – [EMQX](https://www.emqx.io/),  [Mosquitto](https://github.com/eclipse/mosquitto), [NanoMQ](https://nanomq.io/) and [VerneMQ](https://github.com/vernemq/vernemq), were chosen to perform the benchmark test. 
>
> This blog series presents the results of the benchmark test and aims to help users choose a suitable MQTT broker based on their needs and use cases.

In this first post, we provide the benchmarking results of NanoMQ and Mosquitto. A comparison of their features and capabilities can be found in another [blog post.](https://www.emqx.com/en/blog/mosquitto-vs-nanomq-2023-mqtt-broker-comparison) 
...
XMeter provides both a private deployment version (on-premise) and a public cloud SaaS version. In this testing, a private XMeter is deployed in the same VPC as the MQTT broker server.
