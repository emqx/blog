...
MQTTX 1.13.0 introduces the new Payload Inspector. During [MQTT](https://www.emqx.com/en/blog/the-easiest-guide-to-getting-started-with-mqtt) development and testing, you often need to analyze changes in device-reported data, compare message content at different times, or quickly locate fields in complex JSON. To address this, we provide two visualization modes, Diff View and JSON Tree View, significantly improving message analysis efficiency.
...
Whitespace in [MQTT topics](https://www.emqx.com/en/blog/advanced-features-of-mqtt-topics) is legal but usually indicates user input errors. For example, `sensor/temperature ` and `sensor/temperature` are two different topics—this difference is hard to spot visually but can cause messages to fail routing correctly.
...
- **Configurable Disconnect Properties**: Support [MQTT 5.0](https://www.emqx.com/en/blog/introduction-to-mqtt-5) disconnect property configuration
- **IoT Scenario Data Simulation**: Integrate more IoT scenario data simulation capabilities
- **[Sparkplug B](https://www.emqx.com/en/solutions/mqtt-sparkplug) Support**: Extend MQTTX to support Sparkplug B protocol
- **Plugin System**: Support protocol extensions like CoAP, [MQTT-SN](https://www.emqx.com/en/blog/connecting-mqtt-sn-devices-using-emqx), Kafka, MessageQueue
