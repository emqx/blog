...
From the very beginning, Neuron has supported MQTT as one of its communication protocols. The Neuron [MQTT plugin](https://docs.emqx.com/en/neuronex/latest/) allows users to quickly build IoT applications that use MQTT communication between devices and the cloud. 

Neuron provides a [Beckhoff ADS plugin](https://docs.emqx.com/en/neuronex/latest/) from version 2.2.0. The Neuron Beckhoff ADS plugin implements the ADS protocol over TCP. It supports communication with [Beckhoff TwinCAT](https://www.beckhoff.com/en-us/products/automation/twincat/#stage-special-item-s320986-2_t0) PLCs, further enriching Neuron’s connectivity capabilities and resolving user needs. 
...
EMQX has a rich and powerful feature set, such as the SQL-based [rules engine](https://www.emqx.com/en/features/data-processing) to extract, filter, enrich, and transform IoT data in real-time, and data integration to connect EMQX to external data systems like databases.
...
Consult the [installation instruction](https://docs.emqx.com/en/neuronex/latest/) on how to install Neuron. After Neuron is installed, you can access the dashboard through your browser at `http://localhost:7000` (replace "localhost" with your actual IP address).
