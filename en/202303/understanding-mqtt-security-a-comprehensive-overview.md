...
**Application layer:** Although it seems that we have provided enough security protection at the transport layer, not all systems support TLS. The [MQTT protocol](https://www.emqx.com/en/blog/the-easiest-guide-to-getting-started-with-mqtt) running on the application layer also provides support for password authentication and token authentication through the username and password fields, ensuring that only legitimate devices can access the [MQTT broker](https://www.emqx.com/en/blog/the-ultimate-guide-to-mqtt-broker-comparison). [MQTT 5.0](https://www.emqx.com/en/blog/introduction-to-mqtt-5) also introduces an enhanced authentication mechanism to provide two-way identity confirmation.
