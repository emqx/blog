In the previous articles, we introduced how to connect our GCP [IoT Core](https://www.emqx.com/en/blog/understanding-aws-iot-core) devices to EMQX Cloud and secure the connections via TLS/SSL ports.

GCP IoT Core requires that each device must prepare a JSON Web Token (JWT, RFC 7519). JWTs are used for short-lived authentication between devices and the [MQTT](https://www.emqx.com/en/blog/the-easiest-guide-to-getting-started-with-mqtt) bridges. When creating an [MQTT client](https://www.emqx.com/en/blog/mqtt-client-tools), the JWT must be passed in the password field of the CONNECT message. When connecting over HTTP, a JWT must be included in the header of each HTTP request.
