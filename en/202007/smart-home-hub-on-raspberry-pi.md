...
The communicating protocols between smart home devices are **MQTT**, **Wi-Fi**, **Bluetooth**, etc. The [MQTT protocol](https://www.emqx.com/en/mqtt-guide) is an IoT communicating protocol based on publish/subscribe model, which is simple and easy to implement, supporting QoS and with the small packet. In this article, we will use the [MQTT protocol](https://www.emqx.com/en/blog/the-easiest-guide-to-getting-started-with-mqtt) as the communicating protocol between smart home devices.

Because of the limited RAM and processing power of raspberry Pi we choose the [EMQX Edge](https://www.emqx.com/en/products/emqx) open-sourced by [EMQ](https://www.emqx.com/en) as the [MQTT broker](https://www.emqx.com/en/blog/the-ultimate-guide-to-mqtt-broker-comparison). EMQX Edge is the lightweight IoT edge caculating message middleware and supports the IoT edge hardware.
