...
如何构建一座高效、可靠的数据桥梁，将存储在 MySQL 数据库中的 IT 数据，同步至[工业物联网](https://www.emqx.com/zh/blog/iiot-explained-examples-technologies-benefits-and-challenges)（IIoT）平台是一个普遍存在的难题。传统的固定周期全量轮询（`SELECT * FROM ...`）方案，不仅对生产数据库造成不必要的性能压力，还会产生大量的数据冗余。因此，寻求一种更为优雅且高效的解决方案至关重要。
...
此外，请准备 [MQTT 客户端](https://www.emqx.com/zh/blog/mqtt-client-tools)（例如 [MQTTX](https://mqttx.app/zh)）用于后续的数据验证。
...
为该规则添加一个 **MQTT Sink**，并配置目标 [MQTT Broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison) 的地址（`tcp：//broker.emqx.io：1883`）和主题（`topic/mysql`）。
