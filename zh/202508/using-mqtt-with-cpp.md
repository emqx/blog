...
| MQTT 协议支持 | 支持 MQTT v3.1、v3.1.1 和 v5.0（截至 1.5 版） 完全支持 QoS 0、1 和 2。同步确认。 | 支持 MQTT v3.1、v3.1.1 和 v5.0。 支持 QoS 0、1 和 2。同步确认。 | 仅完全支持 MQTT v5.0（不支持 v3.1/v3.1.1）。 支持 QoS 0、1 和 2。 | 支持 MQTT 3.1.1、5.0 以及基于 MsQuic 的 MQTT over [QUIC](https://www.emqx.com/zh/blog/quic-protocol-the-features-use-cases-and-impact-for-iot-iov)。 支持 QoS 0、1 和 2，并对 QoS 1/2 提供异步确认以提高吞吐量。 支持复杂的多流和优先级功能，能够应对网络状况不佳的问题。 |
...
MQTT 与 C++ 结合使用，能够助力开发者构建高效、可扩展的物联网应用。Paho MQTT C++ 库与 EMQX 强大的代理功能相结合，为实时消息传递奠定了坚实的基础。无论您是开发智能家居、[工业物联网](https://www.emqx.com/zh/blog/iiot-explained-examples-technologies-benefits-and-challenges)还是车联网，这一组合都能确保可靠性和性能。
