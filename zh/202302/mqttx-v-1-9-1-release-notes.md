经过两个 Beta 版本迭代，近日，[MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 5.0 客户端工具 MQTTX 正式发布了 1.9.1 稳定版本。
...
新建一个本地 [MQTT Broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison) 连接，并订阅一个 `mqttx/bench` 主题，然后使用 bench 命令，每秒向该主题发送 1000 条消息，消息内容为一个 `hello` 的字符串文本，QoS 为 0，持续时间为 1 分钟。在每秒接收 1000 条消息的场景下，我们使用 MQTTX 开发环境中的调试工具时来观察和监控 CPU 占用率与内存占用的变化情况。
