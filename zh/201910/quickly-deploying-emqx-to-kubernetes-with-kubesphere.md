...
本文先介绍 KubeSphere 支持的 **第一种应用部署与管理方式**，通过在企业空间导入第三方应用仓库，快速部署一个 [EMQ X](https://www.emqx.io) 集群至 Kubernetes，最终访问 EMQ X 的 Dashboard 服务，同时介绍 EMQ X 本身的特性与部署方式。
...
本文介绍了 KubeSphere 支持的 **第一种应用部署与管理方式**，通过在企业空间导入了 [EMQ X](https://www.emqx.io) 应用仓库，并快速部署到了 Kubernetes。未来，**EMQ X + KubeSphere** 可作为物联网应用场景下快速落地**容器微服务**的解决方案。
