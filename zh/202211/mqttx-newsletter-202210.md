十月初，MQTTX 团队发布了 1.8.3 版本。对桌面端应用实现了 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 5.0 版本适配，MQTTX CLI 支持了多主题订阅，同时修复了多个已知问题。此外，团队正专注于 1.9.0 版本的开发，最主要的更新是为 MQTTX CLI 加入了新的命令 – `bench`，即 MQTT 性能测试命令，帮助用户可以创建、订阅和发布自定义数量的连接、主题与消息。安装或更新后即可快速使用，无需额外操作。
...
用户使用 `bench` 命令可以使用规定速率，创建自定义数量的连接，订阅自定义数量的主题，向单个或多个主题中发送自定义数量的消息，通过一行命令即可简单的测试单个或集群下的 [MQTT 服务器](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison)的连接性能，消息吞吐量等。例如：
