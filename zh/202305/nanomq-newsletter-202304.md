...
0.18 版本继续对 2 个重要功能进行升级：桥接功能新增自动退避重连参数配置选项，简化 DDS 协议代理功能的编译安装。另外 NanoSDK 新增了 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) over [QUIC](https://www.emqx.com/zh/blog/quic-protocol-the-features-use-cases-and-impact-for-iot-iov)  支持和 C++ 封装。
...
NanoMQ 本身是一个 [MQTT broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison)，各种协议转换代理网关例如 DDS/ZeroMQ 等都打包进其内置的命令行工具 nanomq_cli 中，SOME/IP 也不例外。所以使用此功能时需要首先开启 nanomq_cli。NanoMQ 的 SOME/IP 协议代理功能依赖于 GENIVI/COVESA ([COVESA](https://www.covesa.global/)) 组织开源的 VSOME/IP 库。由于此开源库为 MPL license，所以并未被直接集成到 NanoMQ 项目中，需要预先在系统中安装。
