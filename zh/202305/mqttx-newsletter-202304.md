在四月末，MQTTX  团队发布了全新的 1.9.2 版本，这一版本主要强化了命令行客户端的输出模式支持，使用户可以根据需求自定义数据管道（Data Pipeline），进而更加便捷和高效地处理输出数据。此次更新还为桌面客户端带来了对 YAML 格式数据倒入导出的支持，进一步提高了数据处理的灵活性和丰富性。同时，我们继续优化了桌面客户端的使用体验，为 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 服务与应用的开发者们提供更加稳定、高效且易于使用的工具。
