...
Neuron 2.2 已支持北向应用 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 的离线缓存，可将数据存储在内存中。2.3.0 版本进一步优化此功能，支持将数据存储在磁盘中，方便用户存储更大的数据量。

在 MQTT 处于离线状态时，Neuron 会将数据优先存储在内存中，待 MQTT 恢复在线状态后，再将缓存的数据发送到 [MQTT Broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison) 中。缓存数据的大小由用户在应用配置界面中配置的 Cache size 决定。
