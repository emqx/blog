物联网场景中，各类设备终端的种类繁杂，所使用的通信协议各异，从而使得应用层的数据格式也各不相同。为了帮助用户实现统一数据格式，**[EMQX Cloud](https://www.emqx.com/zh/cloud) 最近推出了自定义函数功能**：根据用户自定义的脚本对设备上报的数据进行预处理，并将脚本返回的任意符合用户使用场景需求类型的数据流流转至消息订阅方。
