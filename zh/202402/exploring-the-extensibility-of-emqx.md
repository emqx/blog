...
1. **客户端事件：**它们涉及到 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 连接生命周期的各个阶段。比如，当收到一个 CONNECT 包时，就会激活与 `client.connect` Hook 点相对应的回调函数。
...
在这个示例中，我们将尝试创建一个插件，它可以用一个特殊的规则来增强 EMQX 的访问控制：客户端只能订阅与模式 `msg/{{user-id}}/whatever` 相匹配的主题，其中 `{{user-id}}` 是从 [MQTT 客户端](https://www.emqx.com/zh/blog/mqtt-client-tools) ID 中截取出来的。
