...
EMQX 内置的会话持久化（Durable Session）功能提供了强大的持久性和高可用性。该功能允许将 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 持久会话（Persistent Session）及其消息存储到磁盘上，并在 EMQX 集群的多个节点之间持续复制会话元数据和 MQTT 消息。
...
属性值可以从 [MQTT 客户端](https://www.emqx.com/zh/blog/mqtt-client-tools)连接信息（如用户名、客户端 ID、TLS 证书）处理生成，也可以从认证成功返回的附带的数据中设置。例如：
