...
**[Datalayers](https://datalayers.cn/)：**是澜图未来（成都）数据科技有限公司开发的一款为[工业物联网](https://www.emqx.com/zh/blog/iiot-explained-examples-technologies-benefits-and-challenges)、车联网、能源行业设计的分布式多模态、超融合数据库，具备时序数据存储、多模融合、键值存储、存算分离、读写分离、自适应压缩、原生 SQL、边云同步和云原生等特点，支持边缘向云端的数据同步，并针对受限设备进行优化。
