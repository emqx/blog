...
Kafka Headers 用于在消息中添加键值对组成的元数据，用来标识消息的类型、来源、目标、时间戳等信息。例如，您可以将客户端属性或 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 5.0 User Property 通过 Headers 透传，并在业务侧进行消息路由、过滤、监控和跟踪等操作，实现更灵活的业务开发。
