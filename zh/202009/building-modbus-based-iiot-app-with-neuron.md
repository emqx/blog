...
以下，我们将通过 [EMQ X Neuron](https://www.emqx.io/cn/products/neuron)、[EMQ X Broker](https://www.emqx.io/cn/products/broker) 构建基于 [Modbus](https://zh.wikipedia.org/zh-cn/Modbus) 的简易 IIoT 应用，并使用 [MQTT X](https://mqttx.app/cn/) 订阅/展示数据。



## EMQ X Neuron简介

EMQ 于近日发布了布署在边缘网关上的 [工业协议接入软件 Neuron](https://www.emqx.io/cn/products/neuron)。作为人与机器之间的桥梁，它可以把 TCP/IP 协议的 0/1 数据，转化重组成通俗易懂的 JSON 格式，并使用 [MQTT 协议](https://www.emqx.io/cn/mqtt) 输出到云端，更好地处理人与物之间的交互。
...
**2020 年 9 月**底，我们将通过集成 [Neuron](https://www.emqx.io/cn/products/neuron)，[Edge](https://www.emqx.io/cn/products/edge) 和 [Kuiper](https://www.emqx.io/cn/products/kuiper) 等软件，实现在边缘端的工业协议解析、数据汇聚和流式处理的一整套边缘解决方案；该方案通过与云端的 EMQ X Broker / Enterprise 等系列产品集成，则可以实现一个端到端的、从边缘到云端的完整工业解决方案。
