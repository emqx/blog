...
[MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt)（Message Queuing Telemetry Transport）是一种轻量级发布-订阅消息协议，最初为资源受限设备以及低带宽、高延迟或不稳定网络而设计。如今，它的应用早已超出 IoT 遥测，广泛用于云原生事件流、微服务集成、AI 数据流、数字孪生和软件定义汽车（SDV）架构。
...
WebSocket 是一种通过单一 TCP 连接实现全双工通信的协议。它以 HTTP 握手开始，将连接升级为 WebSocket，并保持长连接以持续进行双向数据传输，避免反复建连带来的开销。如今，它不仅广泛用于面向浏览器的应用，也越来越多用于后端实时系统，包括流式网关、实时 API 流式传输和 AI 推理流式管道。

WebSocket 协议包括建立连接的握手阶段和交换信息的数据传输阶段。

#### 标准进展

- **MQTT 5.0** 于 **2019-03-07** 成为 **OASIS 标准**。
- **HTTP/2 上的 WebSocket** 在 **RFC 8441（2018-09）** 中完成标准化。
- **HTTP/3 上的 WebSocket** 在 **RFC 9220（2022-06）** 中完成标准化。
...
### MQTT：为物联网高效通信与实时数据骨干而生

MQTT 是一种专为资源受限环境设计的轻量级发布-订阅消息传输协议，非常适合低功耗、低带宽或网络不稳定的场景。同时，它极小的报文开销、基于主题的路由机制和多级 QoS，也使其成为现代分布式系统中解耦式实时数据流转的事件骨干。
...
这些特性使 MQTT 同时适用于设备侧与云原生系统。以下是一些典型应用场景。
...
- **云原生事件流**：以低开销发布订阅管道连接边缘网关、平台服务与分析系统。
- **微服务与异步集成**：跨服务边界解耦生产者与消费者，构建高韧性实时工作流。
- **AI 与数字孪生**：将遥测和状态更新持续流入 AI 推理闭环与数字孪生同步链路。
...
WebSocket 协议通过单个 TCP 连接实现全双工通信。它始于一个 HTTP 握手，将连接从 HTTP 升级到 WebSocket，从而允许不间断的双向数据流，避免重复建立连接的开销。

如今，WebSocket 不仅广泛用于面向浏览器的应用，也越来越多用于后端实时系统，包括流式网关、API 流式传输和 AI 推理流式管道。

**WebSocket 的工作原理**

WebSocket 从握手开始。在经典模型下，客户端会请求从 HTTP/1.1 升级到 WebSocket；在现代协议栈中，WebSocket 也可运行在 HTTP/2 和 HTTP/3（RFC 8441 与 RFC 9220）之上。连接建立后，它会提供持久的双向通道，以实现低延迟数据交换。
...
#### 浏览器运行时限制：背压（Backpressure）

浏览器端的 [`WebSocket` API](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket) 不提供内置背压机制。在高吞吐流场景下，入站消息可能快于应用消费速度，从而导致内存占用上升并影响界面响应。在生产环境中，通常会在应用层补充流控策略（如限速、有界队列或消息采样）来提升系统稳定性。
...
- **协作工具**：支持文档和项目的实时共同编辑。
- **实时数据网关**：通过持久双向通道在平台、服务与边缘系统之间持续传输数据。
- **API 流式传输与 AI 推理流**：支持实时 AI 与事件 API 的逐 token 或分块响应。
- **实时控制系统**：支撑低延迟的指令下发与状态反馈闭环。
...
| **消息分发**             | Broker 可为离线订阅者缓存消息             | 不支持原生消息队列；依赖额外软件。在 WebSocket 中，可靠性、持久化与重放通常由应用层处理。 |
| **消息 QoS**             | 0（最多一次），1（至少一次），2（仅一次） | 无内置 QoS；依赖 TCP             |
| **消息排队**             | 由 Broker 支持                            | 原生不支持                       |
| **标准和协议合规**       | MQTT 5.0 于 2019-03-07 成为 OASIS 标准    | RFC 6455；并由 RFC 8441（HTTP/2，2018-09）和 RFC 9220（HTTP/3，2022-06）扩展 |
...
| **适用场景**             | IoT、遥测、受限网络、云原生事件流、微服务、AI 数据管道、数字孪生、SDV | 适用于实时 Web 应用、游戏和互动平台 |
...
- **MQTT** 最适合采用一对多通信模型的不稳定网络上的低功耗设备；如果您需要在边缘、云服务与数据系统之间构建事件驱动消息骨干，或为 AI 工作流、数字孪生、SDV 以及跨服务发布订阅构建实时数据管道，也更建议选择 MQTT。为了确保与基于 Web 的客户端兼容，MQTT 可以运行在 WebSocket 之上。
...
MQTT 在需要跨大量设备进行可靠、低开销通信的场景中表现突出，WebSocket 则更适合低延迟、强交互的实时体验；通过 MQTT over WebSocket 将两者结合后，可以在 IoT 系统、浏览器与后端服务，以及日益普及的 AI、边缘计算和数字孪生架构中，构建一套实用且安全的持续实时数据交换基础。
