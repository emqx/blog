...
[MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 驱动在 3.5.0 版本中得到了多项功能增强，进一步提升了其在工业互联中的应用能力：

1. [**支持自定义数据上报格式**](https://docs.emqx.com/zh/neuronex/latest/configuration/north-apps/mqtt/api.html#custom-自定义格式)：MQTT 插件将采集到的数据以 JSON 形式发布到 [MQTT Broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison)指定的主题。 上报数据的具体格式由**上报数据格式**参数指定，有**Values-format 格式、Tags-format 格式、ECP-format 格式、Custom 自定义格式**多种格式可选。用户可以通过**Custom自定义格式**上报MQTT 数据，满足不同应用场景的需求。
...
2. **优化点位浏览功能**：NeuronEX 提供了点位发现功能，帮助用户高效管理 [OPC UA](https://www.emqx.com/zh/blog/opc-ua-protocol) 设备的点位。通过扫描 OPC UA 服务器地址空间，用户可以快速发现并添加点位到采集组，实时监控数据，并导出点表以便本地编辑和使用。
