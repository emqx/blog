...
在之前的[《当数据库遇见流计算：流数据库的诞生！》](https://www.emqx.cn/blog/birth-of-streaming-database) 一文中，我们正式提出了「流数据库」这一全新数据库品类。 **在今天这篇文章中，我们将正式向大家介绍 EMQ 正在研发的流数据库产品 HStreamDB。** 相比当前各种非系统化的流数据解决方案，我们相信由 HStreamDB 开创的 [流数据库](http://hstream.io/) 会是实时数据处理时代的最佳选择，也将成为未来企业软件系统的核心基础设施。
