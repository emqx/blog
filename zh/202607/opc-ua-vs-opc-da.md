...
OPC UA 凭借其卓越的性能和灵活性，已在[工业物联网](https://www.emqx.com/zh/blog/iiot-explained-examples-technologies-benefits-and-challenges)（IIoT）、智能制造、远程设备监控等多个领域得到广泛应用。例如，在智能工厂中，OPC UA 能够实现从生产线设备到更高层级管理系统（如 MES、ERP 等）的无缝数据集成。
...
## **EMQX Neuron：支持 OPC 协议的工业网关**



[EMQX Neuron](https://www.emqx.com/zh/products/emqx-neuron) 是专为工业领域设计的软件，专注于设备数据采集和边缘智能分析。它主要部署于工业环境，促进工业设备通信、工业总线协议采集、工业系统数据集成、边缘级数据过滤与分析、AI 算法集成，以及与 [工业物联网平台](https://www.emqx.com/zh/blog/iiot-platform-key-components-and-5-notable-solutions)集成。EMQX Neuron 提供多协议接入能力，支持同时接入如 [Modbus](https://www.emqx.com/zh/blog/modbus-protocol-the-grandfather-of-iot-communication)、[OPC UA](https://www.emqx.com/zh/blog/opc-ua-protocol)、OPC DA、Ethernet/IP、Ethernet/IP、[BACnet](https://www.emqx.com/en/blog/bacnet-protocol-basic-concepts-structure-obejct-model-explained)、西门子、三菱等数十种工业协议。

NeuronEX 的 OPC UA 插件可作为客户端访问 KEPServerEX、工业网关 OPC 服务器、Prosys 模拟服务器和 Ignition 等 OPC UA 服务器。它也可以直接访问硬件设备的内置 OPC UA 服务器，例如西门子 S7-1200 PLC、欧姆龙 NJ 系列 PLC 等的内置服务器。更多信息，您可以查看 [OPC UA | Neuron 文档](https://docs.emqx.com/zh/neuronex/latest/configuration/south-devices/opc-ua/overview.html)以及[在工业物联网中将 OPC UA 数据桥接到 MQTT：详细教程](https://www.emqx.com/zh/blog/bridging-opc-ua-data-to-mqtt-for-iiot)。

EMQX Neuron 支持 OPC DA 数据采集。由于 OPC DA 依赖 Windows COM/DCOM 机制，而 NeuronEX 运行在 Linux 环境中，因此需要通过 EMQ 提供的 Windows 桌面程序 **NeuronHUB** 进行协同采集。

在该方案中，NeuronHUB 部署在现场 Windows 主机上，负责接入 OPC DA 等 Windows 相关协议并完成数据采集；NeuronEX 则通过南向插件 **NEURON HUB** 接入 NeuronHUB 已采集的数据，实现统一的数据接入与转发。更多信息，请参考 [OPC DA | Neuron 文档](https://docs.emqx.com/zh/neuronex/latest/configuration/south-devices/neuhub/opcda.html)。

## [**结语**](https://www.emqx.com/zh/blog/opc-ua-vs-opc-da#结语)



OPC UA 与 OPC DA 各有适用场景：OPC UA 更适合面向未来的跨平台、安全互联需求，OPC DA 则仍广泛存在于大量传统工业系统中。对于企业而言，关键不只是选择某一种协议，而是如何高效、稳定地接入不同现场设备的数据。

EMQX Neuron 面向工业数据采集场景，既支持 OPC UA 数据接入，也可通过 NeuronHUB 协同完成 OPC DA 数据采集，帮助用户将不同协议、不同系统中的数据统一接入、处理并转发到工业物联网平台，加速设备数据上云和数字化应用落地。
