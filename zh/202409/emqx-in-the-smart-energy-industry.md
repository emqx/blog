...
EMQ 自主研发的云原生分布式 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 接入平台 EMQX Platform 能够兼容多种消息传输协议，具备高可用性和扩展性，轻松处理大规模并发消息传输，并提供端到端数据加密和细粒度访问控制功能，充分利用数据价值的同时，全面满足企业数据的合规性需求。
