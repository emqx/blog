将 PLC 数据接入 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 是现代工业数字化的第一步。MQTT 作为轻量级的消息传输协议，凭借其在弱网环境下的可靠传输能力，对实时数据流传输的支持，以及与主流云平台、数据分析工具和 MES 系统的原生兼容性，已成为[工业物联网](https://www.emqx.com/zh/blog/iiot-explained-examples-technologies-benefits-and-challenges)领域的事实标准。

然而，在真实的工厂环境中，将 PLC 数据接入 MQTT 却远比想象中复杂。不同厂商的 PLC 采用不同的通信协议（如 Modbus、[OPC UA](https://www.emqx.com/zh/blog/opc-ua-protocol)、西门子 S7、Ethernet/IP 等），传统方案需要为每种协议单独编写驱动程序，部署和维护成本高昂。
...
- **[MQTT Broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison)**：使用公共 Broker `broker.emqx.io`
- **[MQTT 客户端](https://www.emqx.com/zh/blog/mqtt-client-tools)**：使用 [MQTTX](https://mqttx.app/zh) 验证数据
