...
Azure IoT Hub 并非一个通用的 [MQTT Broker](https://www.emqx.com/zh/blog/the-ultimate-guide-to-mqtt-broker-comparison)。从架构上看，它是一个由持久化存储支撑的专用数据接收网关。消息在得到确认前，会被写入磁盘、复制并持久保存，连接、吞吐量和持久性被紧密耦合。
...
这对于 [MQTT](https://www.emqx.com/zh/blog/the-easiest-guide-to-getting-started-with-mqtt) 负载而言是一个巨大的挑战。MQTT 协议的设计初衷是实现高频、轻量且高效的二进制数据传输。在网络层面，这类负载几乎不占带宽；但在计费层面，Azure 将每一次细微的数据更新都强行计入 4KB 的最小价格。
